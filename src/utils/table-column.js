...
export default {
  ...
  methods: {
    ...
    setColumnForcedProps(column) {
      // 对于特定类型的 column，某些属性不允许设置(三种type：1.selection多选框，2.index显示索引，3.expand展开行)
      const type = column.type; 
      // cellForced是一个对象，里面包含了三种type对应的render方法及一些属性(sortable,resizeable,className...)
      const source = cellForced[type] || {};
      Object.keys(source).forEach(prop => {
        let value = source[prop];
        if (value !== undefined) {
          // 只有当type = expand 的时候才有className,这里合并用户定义的className和'el-table__expand-column'
  ...
  created() {
    ...
    // column是一个对象，key为属性名，值为用户传进来的值
    let column = this.getPropsData(basicProps, sortProps, selectProps, filterProps);
    // 合并默认值与用户传进来的值(mergeOptions里面主要是过滤了undefined的值)
    column = mergeOptions(defaults, column);

    // 注意 compose 中函数执行的顺序是从右到左(返回一个函数)
    /* 
      先合并自身className和用户设置的className,还有一些不能被覆盖的属性
      再给column设置width
    */
